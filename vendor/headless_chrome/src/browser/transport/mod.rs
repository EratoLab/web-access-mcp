        ...
        C: Method + serde::Serialize,
    {
        ...
        match destination {
            MethodDestination::Target(session_id) => {
                ...
                let target_call = target_method.to_method_call(self.unique_call_id());
                let target_message_text = serde_json::to_string(&target_call)?;
                if let Err(e) = self.web_socket_connection.send_message(&target_message_text) {
                ...
                trace!("sent method call to target via browser websocket");
            }
